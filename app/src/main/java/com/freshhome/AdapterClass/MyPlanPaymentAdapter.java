...
public class MyPlanPaymentAdapter extends RecyclerView.Adapter<MyPlanPaymentAdapter.MyViewHolder> {
    Context context;
    LayoutInflater inflater;
    ArrayList<myCards> arrayListcards;

    public MyPlanPaymentAdapter(Context context, ArrayList<myCards> arrayListcards) {
        ...
        mViewHolder.switch_active.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                for (int j = 0; j < arrayListcards.size(); j++) {
                    ...
                    if (isChecked) {
                        ChooseCard.selectedcardid(arrayListcards.get(j).getCard_id(), arrayListcards.get(j).getMasked_num());
                    } else {
                        ChooseCard.selectedcardid("", "");
                    }
                }
